...
describe('compileSchemasFromDirectory', () => {
  ...
  it('compiles matching schemas to self-contained declaration plans', async () => {
    ...
    await writeSchema('generated/ignored.schema.json', { type: 'boolean' });
  ...
  it('accepts multiple glob patterns and forwards compile options', async () => {
    await writeSchema('first.primary.json', true);
    await writeSchema('nested/second.secondary.json', false);
    await writeSchema('ignored.json', { type: 'string' });

    const result = await compileSchemasFromDirectory({
      schemaDirectory: Path.join(testDirectory, 'schemas'),
      outputDirectory: Path.join(testDirectory, 'types'),
      schemaGlob: ['*.primary.json', '**/*.secondary.json'],
      compileOptions: { anyType: 'unknown' },
    });

    expect(result.files.map((file) => file.relativeDeclarationPath)).toEqual([
      'first.primary.d.ts',
      'nested/second.secondary.d.ts',
    ]);
    expect(result.files[0].text).toContain('export type First = unknown;');
    expect(result.files[0].text).not.toContain('type JSONValue');
  });
